...
function dice(){
  let tiro = Math.ceil(Math.random()*6);
  return tiro
};
...
/* ESERCIZIO 8
  ...
  num8 è il numero di volte
  dado è il numero , * num8
  direi che facciamo prima l'array values, ci push dado, for i= num8
*/
console.log('es8');
...
function rollTheDices(num8){
  ...
  oggetto.sum= oggetto.values.reduce((totale, el) => totale + el)
  return oggetto
}
...
function howManyDays (dataPassata){
  ...
  let differenzaMillisec = dataOggi - dataPassata;
  return Math.floor(differenzaMillisec / (1000 * 60 * 60 * 24)); 
}

console.log(howManyDays(new Date('2023-11-11')))
...
function isTodayMyBirthday(bday){
  ...
  let oggiDay = oggi.getDate()
  let oggiMonth = oggi.getMonth()
  if(bdayDay === oggiDay && bdayMonth === oggiMonth) {
    console.log('oggi è il tuo compleanno');
  } else {
    console.log('oggi non è il tuo compleanno');
  } };

  isTodayMyBirthday(new Date ('1994-11-13'))
...
console.log(deleteProp(ogg11, 'nome'));
...
*/
/* prende array
fa map in cui push le chiavi anno
*/
console.log('es14');
